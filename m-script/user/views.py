...
#         profile.available_balance -= profile.plan_amount
...
def user_index(request):
    ...
    if profile.plan_name:
        ...
        if time_difference > 0:
            ...
            # added bonus to book balance
            book_balance = profile.book_balance + live_profit + profile.bonus
...
def user_plans(request):
    ...
    if request.method == 'POST':
        ...
        profile = Profile.objects.get(user=user)
        profile.save
        if Decimal(amount) > profile.available_balance:
...
#         #     return render(request,'user/index.html',context)
#         email_address = request.POST['wallet_address']
#         try:
...
#                 messages.info(request, 'You did not include an email address')
#                 return redirect('/user/')
        
#             if int(float(usdt_amount)) > profile.available_balance:
#                 messages.info(request, 'You have insufficient funds')
#                 return redirect('/user/')
            
                
#             transfer = Transfer.objects.create(profile=profile, amount=amount, usdt_amount=usdt_amount, username=username)
#             transfer.save()
#             action = f'You has transferred {amount} {wallet_type} to {transfer_user.username}'
...
#             transaction.save()
#             body = f'{profile.user.username} has transferred {transfer.amount} {transfer.wallet_type} to {transfer.email_address}'
#             body_two = f'You has transferred {transfer.amount} {transfer.wallet_type} to {transfer.email_address}'
#             body_three = f'{profile.user.username} has transferred {transfer.amount} {transfer.wallet_type} to you'
...
#             # send_mail(subject=subject,message=body_two,from_email=settings.EMAIL_HOST_USER,recipient_list=[request.user.email])
#             send_email(subject,body_two,request.user.email)
#             #send email to transfer user 
#             #send_mail(subject=subject,message=body_three,from_email=settings.EMAIL_HOST_USER,recipient_list=[transfer_user.email])
#             send_email(subject,body_three,transfer_user.email)
...
def user_transfer(request):
    ...
    sender = Profile.objects.get(user=sender_user)
    notifications_unread_count = Notification.objects.filter(
        profile=sender, read=False).count()
    notifications_unread = Notification.objects.filter(
        profile=sender, read=False).order_by("-time")[:3]
    transfers = Transfer.objects.filter(
        profile=sender).order_by("-time")[:5]
    ...
    if request.method == "POST":
        ...
        try:
            ...
            t_user = User.objects.get(username=receiver_username)
            transfer_user = Profile.objects.get(user=t_user)
            amount = request.POST.get("amount", None)
            username = request.POST.get("username", None)
            usdt_amount = request.POST.get("usdt_amount", 0)
            if usdt_amount > sender.available_balance:
                messages.info(request, "You have insufficient funds")
                return redirect("/user/")
            ...
            body = f"{sender.user.username} has transferred {transfer.amount}USD to {transfer_user.user.username}"
            body_two = f"You have transferred {transfer.amount}USD to {transfer_user.user.username}"
            body_three = f"{sender.user.username} has transferred {transfer.amount} USD to you"
            subject = "Transfer Requested"
            send_email(subject, body, settings.RECIPIENT_ADDRESS)
            send_email(subject, body_two, request.user.email)
            send_email(subject, body_three, transfer_user.user.email)
            messages.info(request, 'You have applied for a transfer')
            return redirect('/user/transfer')
